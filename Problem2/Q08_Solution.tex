...
分子：(c) H$_2$O，DMET 按原子分片。


\section{(a) Schmidt 秩上界与浴数上界}

\subsection{一般量子态的 Schmidt 秩上界}
...
\subsection{HF 态的更紧上界：$\gamma^2 = \gamma$ 的作用}
...
\section{(b) DMET 最小浴精确复现 HF 密度矩阵}

\subsection{第一步：构造浴轨道}
...
\subsection{第二步：用 $\gamma^2 = \gamma$ 计算耦合强度}
...
\subsection{第四步：证明 $\gamma|_{\text{embed}}$ 是嵌入空间的 HF 解}
